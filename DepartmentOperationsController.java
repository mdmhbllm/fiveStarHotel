package com.example.fivestarthotel.GeneralManager;

import javafx.event.ActionEvent;
import javafx.scene.control.*;

public class DepartmentOperationsController
{
    @javafx.fxml.FXML
    private TableColumn pendingIssuesCol;
    @javafx.fxml.FXML
    private DatePicker dataRangeDatePicker;
    @javafx.fxml.FXML
    private TableColumn departmentNameCol;
    @javafx.fxml.FXML
    private ComboBox departmentSelectionComboBox;
    @javafx.fxml.FXML
    private TableColumn currentStatusCol;
    @javafx.fxml.FXML
    private TableColumn totalStaffOnDutyCol;
    @javafx.fxml.FXML
    private TableView departmentTableView;
    @javafx.fxml.FXML
    private TextField totslStaffOnDutyTF;
    @javafx.fxml.FXML
    private TextField currentStatusTF;
    @javafx.fxml.FXML
    private TextField pendingIssuesTF;
    @javafx.fxml.FXML
    private TextField departmentNameTF;

    @javafx.fxml.FXML
    public void initialize() {
    }

    @javafx.fxml.FXML
    public void viewOA(ActionEvent actionEvent) {
    }

    @javafx.fxml.FXML
    public void returnToDashboardOA(ActionEvent actionEvent) {
    }
}