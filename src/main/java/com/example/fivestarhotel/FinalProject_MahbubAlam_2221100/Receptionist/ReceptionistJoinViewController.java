...
import com.example.fivestarhotel.FinalProject_MahbubAlam_2221100.Guest.GuestSignInViewJoiningController;
...
public class ReceptionistJoinViewController {
    @javafx.fxml.FXML
    ...
    public void joinButtonOnAction(ActionEvent actionEvent) throws IOException {
        ...
        Receptionist receptionist = new Receptionist(fullName, email, password, phone, "Receptionist",address, country, zip, city);
        DataManager.saveObject(receptionist, "Receptionist.bin");


        FXMLLoader fxmlLoader =  new FXMLLoader(JoinController.class.getResource("receptionistSignInViewJoining.fxml"));;
        Scene scene = new Scene(fxmlLoader.load());
        Stage newStage = (Stage)(((Node)actionEvent.getSource()).getScene().getWindow());
        newStage.setScene(scene);

        ReceptionistSignInViewIfJoining nextController = fxmlLoader.getController();
        nextController.receiveInformation(receptionist.getUserId(), fullName,receptionist.getPassword());
        newStage.show();
    }
